...
#################### REPLAY ####################
REPLAY_LAYERS = 7
REPLAY_THRESHOLDS = {'layer_0': 4.812, 'layer_1': 4.781, 'layer_2': 3.734, 'layer_3': 3.297, 'layer_4': 3.328, 'layer_5': 3.812, 'layer_6': 4.312}
captured_states = {}
entropy_hook_mask = {}
...
def get_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Debug mode: first subset JSON only (sorted), at most 4 samples, single GPU; use --chunk_idx 0 only. "
...
def setup_model(model_path, base_model_path, processor_model_path, device):
    ...
    # Processor/tokenizer from a full Qwen2.5-VL repo; many RL-only repos omit preprocessor_config.json.
...
def evaluate(data, video_root, slurm_procid, args, subset=None):
    ...
    accs = process_work_items(
        ...
        "cuda",
        f"{args.output_prefix}_{slurm_procid}_{subset.lower()}",
...
DATA_LIST = {
    "4_count": "Evaluation/MLVU/video/4_count",
    "3_ego": "Evaluation/MLVU/video/3_ego",
    "2_needle": "Evaluation/MLVU/video/2_needle",
    "5_order": "Evaluation/MLVU/video/5_order",
    "1_plotQA": "Evaluation/MLVU/video/1_plotQA",
    "6_anomaly_reco": "Evaluation/MLVU/video/6_anomaly_reco",
    "7_topic_reasoning": "Evaluation/MLVU/video/7_topic_reasoning"
}
...
if __name__=='__main__':
    ...
    paths = 'replay_eval/jsons/mlvu'

    json_files = sorted(f for f in os.listdir(paths) if f.endswith('.json'))
    if args.dev:
        json_files = json_files[:1]
        num_gpus = 1
        print("DEV mode: first subset only, at most 4 samples; use --chunk_idx 0 only.")
        if args.chunk_idx != 0:
    ...
    for dd in json_files:
        subset_key = dd.replace('.json', '')
        dd_root = DATA_LIST[subset_key]
        ...
        if args.dev:
            data = data[:4]
            print(f"DEV mode: running inference on {len(data)} sample(s) (cap 4) for {dd}.")

        data_chunks = split_data(data, num_gpus)
        current_data_chunk = data_chunks[args.chunk_idx]

        acc = evaluate(current_data_chunk, dd_root, args.chunk_idx, args, subset=dd.replace('.json', '').split('_')[-1])
        print(acc)