...
def get_args():
    ...
    parser.add_argument(
        "--dev",
        action="store_true",
        help="Debug mode: at most 4 samples, single GPU; use --chunk_idx 0 only. Omit for full evaluation.",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
...
def create_work_items(data, video_root):
    ...
    for i, info in enumerate(data):
        video_path = os.path.join(video_root, info['video ID'] + '.mp4')
...
def process_work_items(
    ...
    for idx, item in enumerate(pbar):
        ...
        example_prompt = example_prompt + option
        ...
        item_res = {'video_path': video_path, 'prompt': example_prompt, 'gt': item["solution"], 'pred': answers, 'acc': accs}
...
if __name__=='__main__':
    ...
    path = 'replay_eval/jsons/test_Video-Holmes.json'
    root = 'Evaluation/Video_Holmes'
