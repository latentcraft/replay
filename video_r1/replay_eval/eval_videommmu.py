...
def evaluate(data, video_root, slurm_procid, args, adaption=False, subset=None):
...
if __name__=='__main__':
    ...
    subset_dir = 'replay_eval/jsons/videommmu'
    subset_files = sorted(f for f in os.listdir(subset_dir) if f.endswith('.json'))
    if args.dev:
        subset_files = subset_files[:1]
        num_gpus = 1
    ...
    for subset in subset_files:
        path = os.path.join(subset_dir, subset)
        root = 'Evaluation/VideoMMMU'
        ...
        if args.dev:
            data = data[:4]
            print(f"DEV mode: running inference on {len(data)} sample(s) (cap 4) for {subset}.")

        data_chunks = split_data(data, num_gpus)
        current_data_chunk = data_chunks[args.chunk_idx]
        adaption = "Adaptation" in subset

        acc = evaluate(current_data_chunk, root, args.chunk_idx, args, adaption, subset=subset)
        print(acc)