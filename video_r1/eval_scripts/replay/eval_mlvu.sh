...
# HF ids: RL weights MODEL_PATH; base replay BASE_MODEL_PATH; processor/tokenizer PROCESSOR_MODEL_PATH (Qwen2.5-VL full repo).
...
# Default: full eval. Smoke test: bash eval_mlvu.sh 0 --dev (first arg is chunk_idx, rest passed to python).
if [ "$#" -ge 1 ]; then shift; fi

python replay_eval/eval_mlvu.py \
    --model-path "$MODEL_PATH" \
    --base-model-path "$BASE_MODEL_PATH" \
    --processor-model-path "$PROCESSOR_MODEL_PATH" \
    --output-prefix ../OUTPUT/Video-R1/replay/Video-R1-7B/mlvu/"$TIMESTAMP" \
    --chunk_idx "$CHUNK_IDX" \
    --dev