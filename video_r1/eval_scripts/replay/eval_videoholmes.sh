...
# Default: full eval. Smoke test: bash eval_videoholmes.sh 0 --dev
if [ "$#" -ge 1 ]; then shift; fi

python replay_eval/eval_videoholmes.py \
    --model-path "$MODEL_PATH" \
    --base-model-path "$BASE_MODEL_PATH" \
    --processor-model-path "$PROCESSOR_MODEL_PATH" \
    --output-prefix ../OUTPUT/Video-R1/replay/Video-R1-7B/videoholmes/"$TIMESTAMP" \
    --chunk_idx "$CHUNK_IDX" \
    --dev
