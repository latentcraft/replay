...
def inference(image_path, prompt, answer, model, processor, max_new_tokens=10240, device="cuda", pred_glue=None):
...
def create_work_items(data, image_root):
    ...
    for i, info in enumerate(data):
        image_path = os.path.join(image_root, info['image'])
        
        # https://github.com/Visual-Agent/DeepEyes/pull/66
        # 解析问题文本，提取问题和选项
        text = info['text']
        ...
        # 分离问题和选项部分
        # 匹配问题部分（在第一个选项之前）
        question_match = re.match(r'(.*?)\([A-Z]\)', text, re.DOTALL)
        if question_match:
        ...
        # 提取结尾的说明文字（如果有）
        instruction_match = re.search(r'\n(Answer with.*?)$', text, re.DOTALL)
        instruction_text = instruction_match.group(1).strip() if instruction_match else ""
        ...
        for label, option_text in options:
            ...
            if label == original_label:
                ...
                other_options.append(option_text)
        
        # 重新组织选项，正确答案放在第一位
        reordered_options = [correct_answer_text] + other_options
        
        # 构建新的问题文本
        new_text = question_text + "\n"
        # 动态生成选项标签（从A开始）
        for idx, option_text in enumerate(reordered_options):
            option_label = chr(ord('A') + idx)  # A, B, C, D, E, F, ...
            new_text += f"({option_label}) {option_text}\n"
        
        if instruction_text:
            new_text += instruction_text
        
        example = {
            "problem": {"question": new_text.strip()},
            "solution": {"answer": "A"},
            "image_path": image_path,
            "image_id": info['question_id']
        }
...
def process_work_items(work_items, model_base, device, checkpoint_dir):
    ...
    for idx, item in enumerate(pbar):
        ...
        question = item['problem']['question'].split('\n')[0]
        options = item['problem']['question'].split('\n')[1:-1]
        ...
        ans = inference(image_path, prompt, item['solution']['answer'], model, processor, device=device, pred_glue=pred_glue)
        print(ans['pred_ans'])
        ...
        item_res = {'image_path': image_path, 'prompt':prompt, 'gt': item["solution"], 'pred': answers, 'acc': accs}
        append_to_jsonl(log_path, item_res)

    del model, processor
    return accs

def evaluate(data, image_root, slurm_procid, args):
    work_items = create_work_items(data, image_root=image_root)
...
if __name__=='__main__':
    ...
    path = 'Evaluation/vstar/test_questions.jsonl'
    root = 'Evaluation/vstar'
