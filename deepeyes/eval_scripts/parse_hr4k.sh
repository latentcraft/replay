...
if [ -z "$1" ]; then
    ...
    echo "Example: bash eval_scripts/acc.sh DeepEyes-7B hr4k"
    echo "Example: bash eval_scripts/acc.sh DeepEyes-7B hr8k"
    exit 1
fi
...
for file in $files; do
    if [ -f "$file" ]; then
        ...
        while IFS= read -r line; do
            subset=$(echo "$line" | grep -o '"category": "[^"]*"' | sed 's/"category": "\([^"]*\)"/\1/')
...
if [ ${#subset_totals[@]} -gt 0 ]; then
    echo "========================================"
    if [ "$subset_type" = "category" ]; then
        echo "Accuracy by Category"
    else
        echo "Accuracy by Subset (from path)"
    fi
    echo "========================================"
    echo ""
